...
red = 255, 0, 0
blue = 30, 144, 255
multi = [(173, 216, 230), (0, 0, 255), (0, 0, 139)]
bg_color = black

#num of blocks and heights
n = 100
min_val = 1
max_val = 100
...
def draw(algo_name, lst):
    ...
    controls = font.render("R - Reset List | SPACE - Start Sorting", 1, multi[0])
    window.blit(controls, (width/2 - controls.get_width()/2 , 45))

    sorting = font.render("I - Insertion Sort | B - Bubble Sort | S - Selection Sort | Z - Shell Sort", 1, multi[0])
...
def draw_list(lst, colors={}, clear_bg=False):
    ...
    for i, val in enumerate(lst):
        ...
        color = multi[i % 3] #how each block has different shades from one another
...
def main():
    ...
    sorting_algo_generator = None #stores generator object while sorting
